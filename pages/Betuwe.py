...
with st.expander("Toggle indices plot from Sentinel-1 reads",expanded=True):
    ...
    if fid_to_plot_tf is not None:
        ...
        upper = base_chart_grd_tf_rvi.encode(y=alt.Y('RVI:Q',title='RVI', scale = alt.Scale(domain=[min_RVI,max_RVI])))
        lower = base_chart_grd_tf_rvi.encode(y=alt.Y('VV/VH:Q',title='VV/VH', scale = alt.Scale(domain=[min_VVVH,max_VVVH])))
